...
seldom 是建立在 unittest 的基础上的自动化测试框架。与 pytest进行对比，无疑相当于像拿一台`电脑`与一颗 intel `CPU` 进行比较，虽然 intel `CPU` 很强大，但我们无法直接拿一个`CPU`打游戏，对吧? pytest 就像一个 `CPU`，虽然很强大，但无法直接拿来做自动化测试，比如配合各种测试库。而seldom不需要额外安装测试库，即可开始编写自动化测试用例。
